...
def read_url_and_parse(year, i, j, k, l):
...
df_col = ['year', 'date', 'field', 'race', 'race_name'
          ...
          , 'sex', 'age', 'trainerA', 'trainerB', 'weight', 'c_weight', 'jackie', 'j_weight'
          , 'odds','popu']
sparse=True
...
for year in tqdm(range(2008, 2019)):
    for i in tqdm(range(1, 11)):
        for j in tqdm(range(1, 11)):
            html = read_url_and_parse(year, i, j, k, l)
                    ...
                    if soup.find_all('div', attrs={'class', 'Result_Guide'})!=[]:
                        ...
                        for m in range(len(soup.find_all('div', attrs='Rank'))):
                            ...
                            try:
                                ...
                                dst['sex'] = list(detailL.contents[0].split()[0])[0]
